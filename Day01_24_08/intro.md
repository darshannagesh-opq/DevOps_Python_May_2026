### **0. How Does a Computer Understand a Program?**

A CPU only understands **0s and 1s** — this is **machine code**. Every program, in any language, eventually becomes this before it runs.

Writing directly in 0s and 1s is hard for humans, so languages evolved in stages.
...
- Human-readable mnemonics instead of binary: `MOV`, `ADD`, `SUB`
- Still tied closely to hardware.
- Example: `MOV AL, 61h`

#### **Stage 3: The Assembler**
- Translates assembly into machine code.
- First "translator" in the chain.

#### **Stage 4: High-Level Languages**
- Languages like C, Java, Python read closer to English.
- Still need to be converted to 0s and 1s eventually.

#### **Stage 5: Compiler vs Interpreter**
- **Compiler**: translates the whole program at once, before running. Creates an executable file. (C, Java)
- **Interpreter**: translates and runs line by line, no separate file. (Python)

| Stage | What it is | Runs directly on CPU? |
|---|---|---|
| Machine Code | Raw 0s and 1s | Yes |
| Assembly | Mnemonics | No (needs assembler) |
| Compiled Language | C, C++ | No (needs compiler) |
| Interpreted Language | Python | No (needs interpreter) |
